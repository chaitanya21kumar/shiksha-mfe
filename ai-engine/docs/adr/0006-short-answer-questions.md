...
- **Grader agreement is a CI property, not an anecdote.**
  `tests/test_grader_parity.py` drives a corpus through this module, through the
  matcher the shipped player actually contains, and through
  `tests/parity/h5p_essay_reference.js` — a checked-in transcription of
  `Essay.getInput`, `Essay.detectExactMatches` and `TextUtilities.isIsolated` — and
...
- **Indic scripts.** H5P's word-delimiter set is Latin-centric and excludes the
  Devanagari danda (`।`), so a Hindi answer whose phrase ends at a danda is not
  word-isolated and does not match. Adding the character on our side would fix
  Hindi in the SCORM package and leave the H5P package unchanged — which breaks the
  invariant that both mark identically, and an inconsistency between the two is
  worse to debug than a limitation they share. Worth raising upstream.
- **H5P consumes its haystack** rather than advancing a cursor, so the start of
  each remainder counts as a word boundary and `"land heatsland heats"` matches
  `"land heats"` where a cursor scan would find nothing. Our ports reproduce that
  exactly; the parity test pins it.
